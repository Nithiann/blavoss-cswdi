...
export class FestivalDetailComponent implements OnInit, OnDestroy {
  ...
  navigateToPay(festival: IFestival) {
    this.router.navigate(['/ticket/pay'], {queryParams: { festival: JSON.stringify({ _id: festival._id, name: festival.name, price: festival.ticketPrice, genre: festival.genre, endDate: festival.endDate }) }});
  }
